...
const [saveSiteNumber, searchSiteNumber] = input
  .shift()
  .split(' ')
  .map((ele) => Number(ele));

const sitePassword = input.splice(0, saveSiteNumber).map((ele) => ele.trim().split(' '));
const searchSite = input.map((ele) => ele.trim());
const sitePasswordMap = new Map(sitePassword);

searchSite.forEach((site) => {
  console.log(sitePasswordMap.get(site));
});
