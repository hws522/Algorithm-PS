...
const pocketmon = input.splice(0, N).map((ele) => ele.trim());
const poketmonMap = new Map(pocketmon.map((v, i) => [v, i + 1]));
const question = input.map((ele) => (isNaN(ele) ? ele.trim() : Number(ele)));

let answer = '';

// console.log(poketmonMap);

question.forEach((ele) => {
  if (isNaN(ele)) {
    answer += `${poketmonMap.get(ele)}\n`;
  } else {
    answer += `${pocketmon[ele - 1]}\n`;
  }
});
