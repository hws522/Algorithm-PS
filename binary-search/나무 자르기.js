...
const [treeCount, treeLength] = input.shift().split(' ');
const treeArray = input.shift().split(' ').map((tree) => Number(tree)).sort((a, b) => a - b);
let low = 0;
let high = treeArray[treeArray.length - 1];
let result = 0;
const cutLength = (mid) => {
    let result = 0;
    treeArray.forEach((ele) => (result += (ele - mid) > 0 ? (ele - mid): 0))
